...
import { GTS_WEIGHTS as WEIGHTS, GTS_WEIGHT_LABELS as WEIGHT_LABELS, GTS_WEIGHT_DESCRIPTIONS as WEIGHT_DESCRIPTIONS, GTS_WEIGHT_EVIDENCE as WEIGHT_EVIDENCE, GTS_SENTIMENT_DATA as SENTIMENT_DATA } from "@/data/gts-weights";
import DealerGate, { isReportUnlocked } from "@/components/DealerGate";
// GTS: All cars are GPF-fitted; checklist uses GTS-specific fields
...
// Live data context — populated by the main Ferrari812GTSReport component
...
const GTS_HERO_IMAGE = "https://ferrari-cdn.thron.com/delivery/public/image/ferrari/812-gts/3zayf6/std/0x0/812-gts.jpg?quality=auto-high&format=auto";
...
const GPF_LABEL: Record<string, { label: string; cls: string }> = {
  ...
  fitted: { label: "GPF FITTED", cls: "check-borderline" }, // GTS: GPF is expected — all cars are fitted
      ...
      <div className="container py-2.5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="text-xs font-mono truncate">
            ...
            <span className="hidden sm:inline text-foreground/60">"GPF Fitted (All GTS)"</span>
          </div>
        </div>
        <Link href={`/812-gts/${topCar.id}`} className="shrink-0 px-3 py-1.5 bg-primary text-primary-foreground text-[10px] font-bold tracking-wide hover:bg-primary/90 transition-colors whitespace-nowrap">
          View Car →
        </Link>
      </div>
    </div>
  ...
  return (
    <section id="overview" className="relative min-h-[85vh] flex items-end overflow-hidden">
      <div
        ...
        style={{ backgroundImage: `url(https://ferrari-cdn.thron.com/delivery/public/image/ferrari/812-gts/3zayf6/std/0x0/812-gts.jpg?quality=auto-high&format=auto)` }}
      />
      ...
      <div className="relative container pb-10 pt-28 md:pb-16">
        <div className="max-w-3xl">
          ...
          <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl font-black text-foreground leading-tight mb-4 md:mb-6">
            Ferrari 812<br />
            <span className="text-primary italic">GTS</span>
          </h1>
          <p className="text-base md:text-lg text-foreground/70 max-w-xl mb-6 md:mb-8 font-light">
            A rigorous analysis of every Ferrari 812 GTS for sale in the UK.
            The last naturally aspirated V12 open-top Ferrari — we build the investment case from first principles.
          </p>
          ...
          <div className="mb-6 border border-emerald-300 bg-emerald-50 px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div className="text-sm text-emerald-900">
              <strong>Investment verdict: Strong Buy.</strong> The 812 GTS is the last naturally aspirated V12 open-top Ferrari ever made. All examples are GPF-fitted but the open-top experience makes this irrelevant. With only ~600 UK-market examples, structural scarcity supports prices. The 'last NA V12 spider' narrative is the strongest investment thesis in the current Ferrari market.
            </div>
          </div>
          {/* Mobile CTA row */}
...
function RankingsSection() {
  ...
  const [dealerUnlocked, setDealerUnlocked] = useState(() => isReportUnlocked("812-gts"));
  ...
  const gatedCarsRankings = CARS_BY_RANK.filter(c => !c.soldDate && c.dealerType === "general-dealer");
  const freeCarsRankings = CARS_BY_RANK.filter(c => !c.soldDate && c.dealerType !== "general-dealer");
    ...
    <section id="rankings" className="py-14 md:py-20 border-t border-border bg-card/30">
      <div className="container">
        ...
        <MarketChangeBanner
          ...
          modelName="812 GTS"
          onCarClick={(id) => { window.location.href = `/812-gts/${id}`; }}
        />
                ...
                <tr data-car-id={car.id} onClick={() => { if (!dealerUnlocked && car.dealerType === 'general-dealer') return; window.location.href = `/812-gts/${car.id}`; }} className={`border-b border-border/50 hover:bg-muted/40 transition-all duration-300 ${car.soldDate ? 'opacity-50' : ''} ${!dealerUnlocked && car.dealerType === 'general-dealer' ? 'opacity-40 blur-[1px] select-none pointer-events-none' : 'cursor-pointer'}`}>
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {(car as any).priceDropDate && (car as any).priceDropAmount && (
                        <span className="px-1.5 py-0.5 text-[9px] font-bold bg-amber-100 text-amber-700 border border-amber-300 rounded-sm animate-pulse" title={`£${(car as any).priceDropAmount.toLocaleString()} price drop on ${(car as any).priceDropDate}`}>
                          ▼ £{Math.round((car as any).priceDropAmount / 1000)}k DROP
                        </span>
                      )}
                    </div>
                  ...
                  <td className="p-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      {!dealerUnlocked && car.dealerType === 'general-dealer' ? (
                        ...
                        <Link href={`/812-gts/${car.id}`} className="text-primary hover:text-primary/80 text-xs font-medium underline underline-offset-2 whitespace-nowrap">
                          View →
                        </Link>
                      )}
    ...
    <div className={`car-card bg-card flex flex-col h-full ${car.rank === 1 ? "rank-1" : ""} ${car.soldDate ? 'opacity-60' : ''}`}>
      ...
      <div className="p-4 md:p-5 border-b border-border">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-1.5 mb-1">
              ...
              {/* Warranty badge */}
              {(car as any).warrantyType === "ferrari-approved" && (
              ...
              {(car as any).priceDropDate && (car as any).priceDropAmount && (
                <span className="px-1.5 py-0.5 text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-300 rounded-sm whitespace-nowrap animate-pulse" title={`Price reduced by £${(car as any).priceDropAmount.toLocaleString()} on ${(car as any).priceDropDate}`}>
                  ▼ PRICE DROP £{Math.round((car as any).priceDropAmount / 1000)}k
                </span>
              )}
              <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded-sm whitespace-nowrap ${VERDICT_CLASS[getCarVerdict(car.totalScoreNorm, car.priceVariance)]}`}>
                {VERDICT_LABELS[getCarVerdict(car.totalScoreNorm, car.priceVariance)]}
        ...
        {car.images && car.images.length > 0 && (
          <div className="mt-3 rounded-md overflow-hidden bg-muted" style={{height: '160px'}}>
            <img
              src={car.images[0]}
              alt={`${car.year} Ferrari 812 GTS ${car.colour}`}
      ...
      <div className="p-4 md:p-5 border-b border-border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5">
          <CheckItem pass={false /* GTS: no pre-GPF */} label="Pre-GPF" />
          <CheckItem pass={car.checklist.frontLift} label="Suspension Lift" />
          <CheckItem pass={car.checklist.carbonPack} label="Carbon Steering Wheel" />
          <CheckItem pass={car.checklist.daytonaSeats} label="Daytona Seats" />
          <CheckItem pass={false /* GTS: use colour score */} label="Special Colour" />
          ...
          <CheckItem pass={car.checklist.tailorMade} label="Atelier Commission" />
        </div>
      </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[10px] text-amber-800 mb-0.5">
                ...
                {(car as any).priceDropDate && " · Recent price drop signals motivated seller"}
              </div>
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-1">
              <Link href={`/812-gts/${car.id}`} className="text-primary text-sm font-medium hover:underline flex-1">
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-6 md:mb-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              Every Ferrari 812 GTS currently for sale in the UK, ranked by value.
...
const SENTIMENT_BY_YEAR: Record<string, number> = {
  ...
  "2025": 2.00,
  "2026": 1.00,
  // Forward projections — sentiment expected to rise as "last NA V12 open-top" narrative matures
  "2028": 1.55,
  "2031": 1.70,
  "2033": 1.80,
  "2036": 1.90,
          ...
          <div className="lg:col-span-2 bg-card border border-border p-4 md:p-6">
            ...
            {showSentiment && (
              <p className="text-[10px] text-muted-foreground mt-2 leading-relaxed">
                <span className="font-semibold" style={{ color: "oklch(0.65 0.18 295)" }}>Purple line</span> = influence-weighted sentiment score (right axis, 0–2.0). 2026 data is measured; 2028–2036 projected based on "last naturally aspirated V12 open-top Ferrari" narrative maturation. Sentiment rising alongside price supports the investment thesis.
              </p>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-card border border-border p-4">
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                <p>Base: <span className="text-foreground font-mono">+4% p.a.</span> (Hagerty V12 Convertible index)</p>
...
function BuyersGuideSection() {
  const { cars: CARS_BY_RANK, marketStats: MARKET_STATS } = useReportData();
  const activeCars = CARS_BY_RANK.filter(c => !c.soldDate);
  const atelierCars = activeCars.filter(c => c.checklist?.tailorMade);
  ...
  const items = [
    { title: "All 812 GTS Are GPF-Fitted", priority: "IMPORTANT", priorityCls: "text-amber-700 border-amber-400/30 bg-amber-400/10", content: "Every 812 GTS was produced from 2020 onwards and is GPF-fitted. Unlike the 812 Superfast (where pre-GPF examples command a premium), all GTS examples are equal in this regard. The investment case rests on open-top exclusivity, colour, and specification." },
    ...
    { title: "ULEZ & Road Tax", priority: "UK-SPECIFIC", priorityCls: "text-muted-foreground border-border", content: "Despite 366g/km CO2, the 812 GTS is Euro 6 compliant and fully ULEZ-exempt. Annual VED is £620 (highest band). No daily charge to drive in London or other UK Clean Air Zones." },
    ...
    { title: "Market Timing", priority: "OPPORTUNITY", priorityCls: "text-green-700 border-green-400/30 bg-green-400/10", content: (() => {
      ...
      return `The 812 GTS is widely considered at the bottom of its depreciation curve in ${MARKET_STATS.lastUpdated.split('-')[0] || '2026'}. With ${activeCars.length} active UK listings ranging from £${Math.round(minP/1000)}k to £${Math.round(maxP/1000)}k, and ${undervaluedCount} currently priced below IIV, the market presents genuine buying opportunities. The 12Cilindri successor costs ~£500k after options. With only ~599 examples produced worldwide, the investment window is now.`;
    })() },
  ];
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-8 md:mb-12">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              The nine things you must know before buying a Ferrari 812 GTS — from GPF status and colour hierarchy to known issues and market timing.
            </p>
          </div>
        </div>
      ...
      <div className="container">
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          <p className="text-base md:text-lg text-muted-foreground max-w-2xl mx-auto">
            ...
            these are the three Ferrari 812 GTSs worth buying right now.
          </p>
        ...
        <div className="border border-border bg-card">
          ...
          <div className={`divide-y divide-border/50 ${
            ...
            {rest.map((car) => (
              <Link key={car.id} href={`/812-gts/${car.id}`} className={`flex items-center gap-3 px-4 py-3 hover:bg-muted/40 transition-colors group cursor-pointer ${car.soldDate ? 'opacity-50' : ''}`}>
                {/* Rank */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-foreground truncate">
                    ...
                    {(car as any).priceDropDate && <span className="ml-1.5 text-[9px] font-bold text-amber-700">▼ DROP</span>}
                  </div>
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-8 md:mb-10">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              Estimate your monthly PCP or HP payments based on the current average 812 GTS asking price.
    ...
    <footer className="border-t border-border py-8 md:py-10">
      <div className="container">
        <div className="flex flex-col md:flex-row justify-between gap-6">
          <div>
            <div className="font-serif text-base md:text-lg font-bold text-foreground mb-1">Ferrari 812 GTS</div>
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function Ferrari812GTSReport() {
  useEffect(() => { document.title = 'Ferrari 812 GTS — UK Investment Analysis'; return () => { document.title = 'SupercarIQ — Ferrari Investment Analysis'; }; }, []);
  const { listings, marketStats, isLoading, error } = useLiveListings("812-gts");
  ...
  return (
    ...
    <ReportGate modelKey="812-gts" modelLabel="Ferrari 812 GTS" heroImageUrl={GTS_HERO_IMAGE}>
      <div className="min-h-screen bg-background">
        <IntelBar />
        <GlobalNav reportTitle="812 GTS" />
        <Hero />
        <FloatingFAB />
