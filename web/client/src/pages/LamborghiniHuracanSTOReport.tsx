/**
 * Lamborghini Huracán STO — UK Investment Analysis
 * Gated page with LLM-assisted content from DB.
 * First non-Ferrari model on the platform.
 */
import { useState } from "react";
import { Link } from "wouter";
import { GlobalNav } from "@/components/GlobalNav";
import { trpc } from "@/lib/trpc";
import { Loader2, Lock, TrendingUp, BarChart2, Star, ChevronRight } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { toast } from "sonner";
import { Streamdown } from "streamdown";

const MODEL_KEY = "huracan-sto";
const MODEL_LABEL = "Lamborghini Huracán STO";
const YEAR_RANGE = "2021–2024";
const ENGINE_SPEC = "5.2L Naturally Aspirated V10 · 640hp · RWD · Race-Derived";
const HERO_IMAGE = "/manus-storage/huracan-sto-hero_87419902.jpg";
...
function EmailGate({ onUnlock }: { onUnlock: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    onUnlock();
    toast.success("Access granted — welcome to the Huracán STO analysis.");
    setSubmitting(false);
  };
      ...
      <section className="relative min-h-[60vh] flex items-end overflow-hidden">
        ...
        <div className="relative container pb-10 pt-28 md:pb-16">
          <div className="max-w-3xl">
            <div className="data-label mb-3 text-amber-500 text-xs">Full UK Market · {MODEL_LABEL} · {YEAR_RANGE}</div>
            <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl font-black text-foreground leading-tight mb-4">
              Huracán<br />
              <span className="text-amber-500 italic">STO</span>
            </h1>
        ...
        <div className="container max-w-2xl">
          <div className="bg-card border border-border p-8 md:p-12">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <div className="data-label text-amber-500 text-xs mb-0.5">GATED ANALYSIS</div>
                <div className="font-serif text-xl font-bold">Full Report Access</div>
              </div>
            </div>
            <p className="text-muted-foreground mb-8 text-sm leading-relaxed">
              Enter your email to access the complete Lamborghini Huracán STO UK investment analysis — 
              IIV scoring, LLM-generated price predictions, influencer sentiment, and live market data.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
              {[
                { icon: <BarChart2 className="w-4 h-4" />, label: "IIV Scoring", desc: "STO-specific scoring: livery, aero, track pack" },
                ...
                <div key={item.label} className="bg-background border border-border p-3">
                  <div className="text-amber-500 mb-1.5">{item.icon}</div>
...
function STOReport() {
  ...
  // Lamborghini uses amber accent instead of Ferrari red
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative min-h-[85vh] flex items-end overflow-hidden">
        ...
        <div className="relative container pb-10 pt-28 md:pb-16">
          <div className="max-w-3xl">
            <div className={`data-label mb-3 ${accent} text-xs`}>
            ...
            <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl font-black text-foreground leading-tight mb-4 md:mb-6">
              Huracán<br />
              <span className={`${accent} italic`}>STO</span>
            </h1>
            ...
            {llmContent?.investmentHeadline && (
              <div className="bg-card border border-border px-4 py-3 mb-6 max-w-xl">
                <div className={`data-label ${accent} text-xs mb-1`}>LLM INVESTMENT VERDICT</div>
      ...
      {llmLoading ? (
        <section className="py-20 flex items-center justify-center">
          <div className="text-center">
            <Loader2 className={`w-8 h-8 animate-spin ${accent} mx-auto mb-4`} />
            ...
            <section className="py-14 md:py-20 border-t border-border">
              <div className="container">
                <div className="flex gap-4 md:gap-8 items-start mb-8">
                  ...
                  <div>
                    <div className={`data-label ${accent} mb-2`}>Investment Analysis</div>
              ...
              <div className="container">
                <div className="flex gap-4 md:gap-8 items-start mb-8">
                  ...
                  <div className="w-full">
                    <div className={`data-label ${accent} mb-2`}>Price Predictions</div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                      {[
                        ...
                        <div key={pred.label} className="bg-card border border-border p-5">
                          ...
                          <div className={`font-serif text-2xl font-bold ${accent}`}>{pred.value}</div>
                        </div>
                      ))}
                    </div>
                    <div className="prose prose-sm max-w-3xl text-foreground/80">
            ...
            <section className="py-14 md:py-20 border-t border-border">
              <div className="container">
                <div className="flex gap-4 md:gap-8 items-start mb-8">
                  ...
                  <div className="w-full">
                    <div className={`data-label ${accent} mb-2`}>Influencer Pulse</div>
        ...
        <section className="py-20 border-t border-border">
          <div className="container text-center">
            <div className={`data-label ${accent} mb-3 text-xs`}>ANALYSIS IN PROGRESS</div>
            ...
            <p className="text-muted-foreground max-w-md mx-auto mb-8 text-sm">
              Our LLM pipeline is generating the complete Huracán STO investment analysis. Data refreshes 3× daily.
            </p>
...
export default function LamborghiniHuracanSTOReport() {
