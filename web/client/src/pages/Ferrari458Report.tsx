...
import { WEIGHTS_458 as WEIGHTS, WEIGHT_LABELS_458 as WEIGHT_LABELS, WEIGHT_DESCRIPTIONS_458 as WEIGHT_DESCRIPTIONS, WEIGHT_EVIDENCE_458 as WEIGHT_EVIDENCE, SENTIMENT_DATA_458, SENTIMENT_YEAR_TREND_458, KEY_THEMES_458 } from "@/data/f458-weights";
...
// Live data context — populated by the main Ferrari458Report component
type ReportData = { cars: GenericCarSpec[]; marketStats: { activeListings: number; lastUpdated: string; lastUpdatedISO: string; priceMin: number; priceMax: number } };
const DEFAULT_MARKET_STATS = { activeListings: 0, lastUpdated: "Loading…", lastUpdatedISO: new Date().toISOString().slice(0, 10), priceMin: 0, priceMax: 0 };
const ReportDataCtx = React.createContext<ReportData>({ cars: [], marketStats: DEFAULT_MARKET_STATS });
const useReportData = () => React.useContext(ReportDataCtx);

const FERRARI_458_HERO_IMAGE = "https://ferrari-cdn.thron.com/delivery/public/image/ferrari/458-italia/3zayf6/std/0x0/458-italia.jpg?quality=auto-high&format=auto";
...
const GPF_LABEL: Record<string, { label: string; cls: string }> = {
...
function IntelBar() {
  const { cars: CARS_BY_RANK, marketStats: MARKET_STATS } = useReportData();
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-40 intel-bar transition-transform duration-300 ${visible ? "translate-y-0" : "translate-y-full"}`}>
      <div className="container py-2.5 flex items-center justify-between gap-4">
        ...
        <Link href={`/458/${topCar.id}`} className="shrink-0 px-3 py-1.5 bg-primary text-primary-foreground text-[10px] font-bold tracking-wide hover:bg-primary/90 transition-colors whitespace-nowrap">
          View Car →
        </Link>
      </div>
    </div>
  ...
  return (
    <section id="overview" className="relative min-h-[85vh] flex items-end overflow-hidden">
      <div
        ...
        style={{ backgroundImage: `url(https://ferrari-cdn.thron.com/delivery/public/image/ferrari/458-italia/3zayf6/std/0x0/458-italia.jpg?quality=auto-high&format=auto)` }}
      />
      ...
      <div className="relative container pb-10 pt-28 md:pb-16">
        <div className="max-w-3xl">
          ...
          <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl font-black text-foreground leading-tight mb-4 md:mb-6">
            Ferrari 458<br />
            <span className="text-primary italic">Italia / Spider</span>
          </h1>
          <p className="text-base md:text-lg text-foreground/70 max-w-xl mb-6 md:mb-8 font-light">
            A rigorous analysis of every Ferrari 458 Italia and Spider for sale in the UK.
          ...
          <div className="mb-6 border border-emerald-300 bg-emerald-50 px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div className="text-sm text-emerald-900">
              <strong>Investment verdict: Buy now.</strong> The Ferrari 458 Italia is the last naturally aspirated V8 Ferrari mid-engine car — a landmark that will only appreciate. Pre-GPF by design (all 458s), single-owner examples with full Ferrari history are already trading at premiums. The collector narrative is firmly established.
            </div>
          </div>
          {/* Mobile CTA row */}
                            ...
                            <div className="grid md:grid-cols-3 gap-4">
                              ...
                              <div className="space-y-3">
                                ...
                                <div>
                                  ...
                                  <a href={(evidence as any).url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary underline underline-offset-2 break-all hover:opacity-80">{evidence.source}</a>
                                </div>
                                <div>
...
function RankingsSection() {
  ...
  const [dealerUnlocked, setDealerUnlocked] = useState(() => isReportUnlocked("458-italia"));
  ...
  const dealerFiltered = dealerFilter === "all" ? CARS_BY_RANK : CARS_BY_RANK.filter(c =>
    dealerFilter === "ferrari-approved" ? (((c.dealerType as string) === "ferrari-approved") || ((c.dealerType as string) === "ferrari-approved")) : (((c.dealerType as string) !== "ferrari-approved") && ((c.dealerType as string) !== "ferrari-approved"))
  );
  ...
  const gatedCarsRankings = CARS_BY_RANK.filter(c => !c.soldDate && (c.dealerType as string) === "general-dealer");
  const freeCarsRankings = CARS_BY_RANK.filter(c => !c.soldDate && (c.dealerType as string) !== "general-dealer");
      ...
      <div className="container">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              Main Dealer ({CARS_BY_RANK.filter(c => !c.soldDate && (((c.dealerType as string) === "ferrari-approved") || ((c.dealerType as string) === "ferrari-approved"))).length})
            </button>
            {/* Independent */}
            <button
            ...
            >
              Independent ({CARS_BY_RANK.filter(c => !c.soldDate && (((c.dealerType as string) !== "ferrari-approved") && ((c.dealerType as string) !== "ferrari-approved"))).length})
            </button>
            {newTodayCount > 0 && (
              <>
        ...
        <MarketChangeBanner
          cars={CARS_BY_RANK as any}
          lastUpdated={MARKET_STATS.lastUpdated}
          modelName="Ferrari 458"
          onCarClick={(id) => { window.location.href = `/458/${id}`; }}
        />
          ...
          <table ref={tableRef} className="w-full text-xs md:text-sm min-w-[640px]">
            ...
            <tbody>
              {activeCars.map((car) => (
                ...
                <tr data-car-id={car.id} onClick={() => { if (!dealerUnlocked && (car.dealerType as string) === 'general-dealer') return; window.location.href = `/458/${car.id}`; }} className={`border-b border-border/50 hover:bg-muted/40 transition-all duration-300 ${car.soldDate ? 'opacity-50' : ''} ${!dealerUnlocked && (car.dealerType as string) === 'general-dealer' ? 'opacity-40 blur-[1px] select-none pointer-events-none' : 'cursor-pointer'}`}>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {(car as any).motExpired && <span className="px-1.5 py-0.5 text-[9px] font-bold bg-red-50 text-red-700 border border-red-200" title="MOT certificate has expired — buyer must arrange new MOT before road use">⚠ MOT EXPIRED</span>}
                    ...
                    <div className="data-label mt-0.5 text-[10px] flex items-center gap-1 flex-wrap">
                      ...
                      {(((car.dealerType as string) !== "ferrari-approved") && ((car.dealerType as string) !== "ferrari-approved")) && (
                  ...
                  <td className="p-3 text-center">
                    <span className={`px-1.5 py-0.5 text-[9px] font-bold rounded-sm whitespace-nowrap ${
                      ...
                      {(((car.dealerType as string) === "ferrari-approved") || ((car.dealerType as string) === "ferrari-approved")) ? "MAIN" : "INDEP"}
                    </span>
                  </td>
                  <td className="p-3 text-center">
                  ...
                  <td className="p-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      {!dealerUnlocked && (car.dealerType as string) === 'general-dealer' ? (
                        <span className="text-muted-foreground/50 text-xs" title="Unlock to view">🔒</span>
                      ) : (
                        <Link href={`/458/${car.id}`} className="text-primary hover:text-primary/80 text-xs font-medium underline underline-offset-2 whitespace-nowrap">
                          View →
                        </Link>
                      )}
                      {(car.dealerUrl || car.listingUrl) && !car.soldDate && (
                        <a href={car.dealerCarUrl || car.dealerUrl || car.listingUrl} target="_blank" rel="noopener noreferrer"
                          ...
                          className="text-[10px] text-muted-foreground hover:text-primary transition-colors" title={(((car.dealerType as string) === "ferrari-approved") || ((car.dealerType as string) === "ferrari-approved")) ? "View on Ferrari Approved" : "View on dealer site"}>
        ...
        {!dealerUnlocked && gatedCarsRankings.length > 0 && (
          <div className="mt-6">
            <DealerGate
              modelKey="458-italia"
              modelLabel="Ferrari 458 Italia"
              freeCount={freeCarsRankings.length}
              gatedCount={gatedCarsRankings.length}
...
function CarCard({ car }: { car: CarSpec }) {
  ...
  const gpfInfo = car.gpfStatus ? GPF_LABEL[car.gpfStatus] : GPF_LABEL["none"];
  return (
    <div className={`car-card bg-card flex flex-col h-full ${car.rank === 1 ? "rank-1" : ""} ${car.soldDate ? 'opacity-60' : ''}`}>
      ...
      <div className="p-4 md:p-5 border-b border-border">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-1.5 mb-1">
              ...
              {false /* 458 has no Atelier option */ && null}
              {car.soldDate && (
        ...
        {car.images && car.images.length > 0 && (
          <div className="mt-3 rounded-md overflow-hidden bg-muted" style={{height: '160px'}}>
            <img
              src={car.images[0]}
              alt={`${car.year} Ferrari 458 Italia ${car.colour}`}
      ...
      <div className="p-4 md:p-5 border-b border-border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5">
          <CheckItem pass={car.checklist.preGpf} label="Pre-GPF (NA V8)" />
          <CheckItem pass={car.checklist.lowMileage} label="Low Mileage (<10k)" />
          <CheckItem pass={car.checklist.ffsh} label="Full Ferrari Service History" />
          <CheckItem pass={car.checklist.accidentFree} label="Accident Free" />
          <CheckItem pass={car.checklist.carbonPack} label="Carbon Pack" />
          <CheckItem pass={car.checklist.daytonaSeats} label="Daytona Seats" />
          <CheckItem pass={car.checklist.frontLift} label="Front Suspension Lift" />
          <CheckItem pass={car.checklist.ccb} label="Carbon-Ceramic Brakes" />
          <CheckItem pass={car.checklist.scuderiaShields} label="Scuderia Shields" />
          <CheckItem pass={car.checklist.originalColour} label="Original Colour" />
        </div>
      </div>
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-1">
              <Link href={`/458/${car.id}`} className="text-primary text-sm font-medium hover:underline flex-1">
              ...
              {car.dealerUrl && (
                ...
                >
                  ...
                  {(((car.dealerType as string) === "ferrari-approved") || ((car.dealerType as string) === "ferrari-approved")) ? "ferrari-approved" : "View Listing"}
                </a>
              )}
            </div>
          </div>
...
function CarsSection() {
  ...
  const [dealerUnlocked, setDealerUnlocked] = useState(() => isReportUnlocked("458-italia"));
  const soldCount = CARS_BY_RANK.filter((c) => c.soldDate).length;
  const freeCars = CARS_BY_RANK.filter(c => !c.soldDate && (c.dealerType === "ferrari-approved" || c.dealerType === "independent-specialist"));
  const gatedCars = CARS_BY_RANK.filter(c => !c.soldDate && c.dealerType === "general-dealer");
  const visibleCars = CARS_BY_RANK.filter((c) => {
    ...
    if (!dealerUnlocked && c.dealerType === "general-dealer") return false;
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-6 md:mb-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              Every Ferrari 458 Italia currently for sale in the UK, ranked by value.
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-border">
          <div className="flex items-center gap-2 flex-wrap">
            {([
              ...
              { key: "ferrari-approved", label: "ferrari-approved", count: CARS_BY_RANK.filter(c => !c.soldDate && (((c.dealerType as string) === "ferrari-approved") || ((c.dealerType as string) === "ferrari-approved"))).length },
...
const SENTIMENT_BY_YEAR: Record<string, number> = {
  ...
  "2025": 2.00,
  "2026": 1.00,
  // Forward projections — sentiment expected to rise as "last NA V8" narrative matures
  "2028": 1.55,
  "2031": 1.70,
  "2033": 1.80,
  "2036": 1.90,
...
function PredictionsSection() {
  ...
  const [selectedId, setSelectedId] = useState<string>(() => String(activeCarsForPred[0]?.id ?? ''));
  const [showSentiment, setShowSentiment] = useState(true);
  const car = CARS_BY_RANK.find((c) => String(c.id) === selectedId) ?? activeCarsForPred[0]!;
        ...
        <div className="flex gap-4 md:gap-8 items-start mb-8 md:mb-12">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              ...
              Base rates derived from Hagerty Market Index, Full Throttle Talk, and comparable naturally aspirated Ferrari V8 trajectories.
            </p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 bg-card border border-border p-4 md:p-6">
            ...
            {showSentiment && (
              <p className="text-[10px] text-muted-foreground mt-2 leading-relaxed">
                <span className="font-semibold" style={{ color: "oklch(0.65 0.18 295)" }}>Purple line</span> = influence-weighted sentiment score (right axis, 0–2.0). 2026 data is measured; 2028–2036 projected based on "last naturally aspirated V8" narrative maturation. Sentiment rising alongside price supports the investment thesis.
              </p>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-card border border-border p-4">
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                <p>Base: <span className="text-foreground font-mono">+4% p.a.</span> (Hagerty Naturally Aspirated V8 index)</p>
...
function BuyersGuideSection() {
  const { cars: CARS_BY_RANK, marketStats: MARKET_STATS } = useReportData();
  const activeCars = CARS_BY_RANK.filter(c => !c.soldDate);
  const atelierCars = activeCars.filter(c => false /* 458 has no atelier */);
  ...
  const items = [
    { title: "Pre-GPF Advantage", priority: "CRITICAL", priorityCls: "text-emerald-700 border-emerald-400/30 bg-emerald-400/10", content: "All 458 Italia models (2009–2015) are pre-GPF. The naturally aspirated 4.5-litre V8 produces the unrestricted exhaust note that collectors prize. This is a significant advantage over the 488 GTB and F8 Tributo, which are all GPF-fitted." },
    ...
    { title: "Colour Hierarchy", priority: "SIGNIFICANT", priorityCls: "text-muted-foreground border-border", content: (() => {
      const specialColourCars = activeCars.filter(c => (c.colour || "") === 'special');
      const desirableColourCars = activeCars.filter(c => (c.colour || "") === 'desirable');
    ...
    { title: "Essential Options", priority: "MUST-HAVE", priorityCls: "text-primary border-primary/30 bg-primary/10", content: (() => {
      const liftCount = activeCars.filter(c => c.checklist.frontLift).length;
      const ccbCount = activeCars.filter(c => c.checklist?.ccb).length;
    ...
    { title: "ULEZ & Road Tax", priority: "UK-SPECIFIC", priorityCls: "text-muted-foreground border-border", content: "Despite 366g/km CO2, the 458 Italia is Euro 5 compliant and fully ULEZ-exempt. Annual VED is £620 (highest band). No daily charge to drive in London or other UK Clean Air Zones. Pre-GPF cars are the most desirable." },
    { title: "Known Issues", priority: "INSPECT", priorityCls: "text-amber-700 border-amber-400/30 bg-amber-400/10", content: "Check for: fuel pump recall (check completed), throttle body issues on early cars, clutch wear on high-mileage examples, and front splitter damage without suspension lift. A Ferrari dealer PPI is mandatory." },
    { title: "Market Timing", priority: "OPPORTUNITY", priorityCls: "text-green-700 border-green-400/30 bg-green-400/10", content: (() => {
      ...
      return `The 458 Italia is widely considered past the bottom of its depreciation curve in ${MARKET_STATS.lastUpdated.split('-')[0] || '2026'}. With ${activeCars.length} active UK listings ranging from £${Math.round(minP/1000)}k to £${Math.round(maxP/1000)}k, and ${undervaluedCount} currently priced below IIV, the market presents genuine buying opportunities. The 488 GTB successor costs ~£120k used. With only ~1,200 UK examples, the investment window is now.`;
    })() },
  ];
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-8 md:mb-12">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              The nine things you must know before buying a Ferrari 458 Italia or Spider — from GPF status and colour hierarchy to known issues and market timing.
            </p>
          </div>
        </div>
      ...
      <div className="container">
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          <p className="text-base md:text-lg text-muted-foreground max-w-2xl mx-auto">
            ...
            these are the three Ferrari 458 Italias worth buying right now.
          </p>
        ...
        <div className="border border-border bg-card">
          ...
          <div className={`divide-y divide-border/50 ${
            ...
            {rest.map((car) => (
              <Link key={car.id} href={`/458/${car.id}`} className={`flex items-center gap-3 px-4 py-3 hover:bg-muted/40 transition-colors group cursor-pointer ${car.soldDate ? 'opacity-50' : ''}`}>
                {/* Rank */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-foreground truncate">
                    ...
                    {(car as any).motExpired && <span className="ml-1.5 text-[9px] font-bold text-red-700">⚠ MOT</span>}
...
const SENTIMENT_DATA = {
  summary: {
    totalSources: SENTIMENT_DATA_458.length,
    totalViewsAnalysed: SENTIMENT_DATA_458.reduce((s, c) => s + (c.views || 0), 0),
    weightedScore: 1.72,
    pctPositive: 91.0,
    pctMixed: 9.0,
    pctNegative: 0.0,
  ...
  platformBreakdown: [
    { platform: "YouTube", count: 9, totalViews: SENTIMENT_DATA_458.reduce((s, c) => s + (c.views || 0), 0), weightedSentiment: 1.72, sentimentPct: 94.0, color: "oklch(0.72 0.12 75)" },
    { platform: "Forums", count: 2, totalViews: 45000, weightedSentiment: 1.50, sentimentPct: 4.0, color: "oklch(0.45 0.06 75)" },
    { platform: "Reddit", count: 1, totalViews: 12000, weightedSentiment: 1.00, sentimentPct: 2.0, color: "oklch(0.25 0.03 75)" },
  ],
  yearTrend: SENTIMENT_YEAR_TREND_458.map(t => ({ year: t.year, score: t.score, count: 2 })),
  topInfluencers: SENTIMENT_DATA_458.map(s => ({
    creator: s.creator,
    platform: s.platform,
    subscribers: String(s.subscribers >= 1000000 ? (s.subscribers/1000000).toFixed(1) + 'M' : (s.subscribers/1000).toFixed(0) + 'K'),
    views: s.views,
    weight: s.weight * 20,
    sentiment: s.sentiment === 'positive' ? 'Strongly Positive' : s.sentiment === 'mixed' ? 'Mixed' : 'Negative',
    quote: s.quote,
    url: s.videoId ? `https://www.youtube.com/watch?v=${s.videoId}` : '#',
    year: s.year,
  })),
  topThemes: KEY_THEMES_458.map(t => ({
    theme: t.theme,
    count: Math.round(t.frequency / 10),
    sentiment: t.sentiment === 'positive' ? 'Strongly Positive' : t.sentiment === 'mixed' ? 'Mixed' : 'Negative',
    note: t.description,
  })),
  keyInsights: KEY_THEMES_458.slice(0, 5).map(t => ({
    label: t.theme,
    text: t.description,
    type: t.sentiment === 'positive' ? 'positive' : 'neutral',
  })),
};
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-8 md:mb-10">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              Estimate your monthly PCP or HP payments based on the current average 458 Italia asking price.
    ...
    <footer className="border-t border-border py-8 md:py-10">
      <div className="container">
        <div className="flex flex-col md:flex-row justify-between gap-6">
          <div>
            <div className="font-serif text-base md:text-lg font-bold text-foreground mb-1">Ferrari 458 Italia / Spider</div>
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function Ferrari458Report() {
  useEffect(() => { document.title = 'Ferrari 458 Italia — UK Investment Analysis'; return () => { document.title = 'SupercarIQ — Ferrari Investment Analysis'; }; }, []);
  const { listings, marketStats, isLoading, error } = useLiveListings("458-italia");
  ...
  return (
    <ReportDataCtx.Provider value={{ cars: listings, marketStats }}>
    <ReportGate modelKey="458-italia" modelLabel="Ferrari 458 Italia" heroImageUrl={FERRARI_458_HERO_IMAGE}>
      <div className="min-h-screen bg-background">
        <IntelBar />
        <GlobalNav reportTitle="458 Italia" />
        <Hero />
        <FloatingFAB />
