...
function buildRadarData(car: GenericCarSpec) {
  const s = car.scores;
  return [
...
function buildPredictionData(car: GenericCarSpec) {
  const preds = car.predictions ?? {};
  return [
    { year: "2026 (Now)", value: car.askingPrice },
    { year: "2028", value: Math.round(car.askingPrice * Math.pow(1.04, 2) * ((car.totalScoreNorm ?? 50) / 70)) },
    { year: "2031 (5yr)", value: preds.base2031 ?? Math.round(car.askingPrice * 1.20) },
    { year: "2036 (10yr)", value: preds.base2036 ?? Math.round(car.askingPrice * 1.45) },
  ];
}
...
export default function Compare() {
  const { listings, isLoading } = useLiveListings("812-superfast");
  ...
  // Initialise selections once data loads
  const resolvedAId = carAId ?? activeCars[0]?.id ?? null;
  const resolvedBId = carBId ?? activeCars[1]?.id ?? null;

  const carA = allCars.find(c => c.id === resolvedAId) ?? activeCars[0];
  const carB = allCars.find(c => c.id === resolvedBId) ?? activeCars[1];

  if (isLoading || !carA || !carB) {
    return (
  ...
  const scoreBarData = SCORE_CATEGORIES.map(cat => ({
    name: cat.label,
    [carA.colour]: (carA.scores ?? {})[cat.key] ?? 0,
    [carB.colour]: (carB.scores ?? {})[cat.key] ?? 0,
    weight: WEIGHTS[cat.key as keyof typeof WEIGHTS] ?? 0,
  }));

  function CarSelector({ value, onChange, exclude }: { value: number | null; onChange: (id: number) => void; exclude: number | null }) {
    return (
      <div className="relative">
        ...
        >
          {allCars.filter(c => c.id !== exclude).map(car => (
      ...
      <div className="container py-10 md:py-14">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="bg-card border border-border p-4">
            ...
            <CarSelector value={resolvedAId} onChange={setCarAId} exclude={resolvedBId} />
          ...
          <div className="bg-card border border-border p-4">
            ...
            <CarSelector value={resolvedBId} onChange={setCarBId} exclude={resolvedAId} />
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border/50">
                {[
                  ...
                  { label: "5-Year ROI (Base)", a: (carA.predictions?.roi5yr ?? Math.round(((carA.predictions?.base2031 ?? carA.askingPrice * 1.20) / carA.askingPrice - 1) * 100)), b: (carB.predictions?.roi5yr ?? Math.round(((carB.predictions?.base2031 ?? carB.askingPrice * 1.20) / carB.askingPrice - 1) * 100)), fmt: (v: number) => `+${v}%`, higher: true },
                  { label: "10-Year ROI (Base)", a: (carA.predictions?.roi10yr ?? Math.round(((carA.predictions?.base2036 ?? carA.askingPrice * 1.45) / carA.askingPrice - 1) * 100)), b: (carB.predictions?.roi10yr ?? Math.round(((carB.predictions?.base2036 ?? carB.askingPrice * 1.45) / carB.askingPrice - 1) * 100)), fmt: (v: number) => `+${v}%`, higher: true },
                  { label: "5-Year Value (Base)", a: (carA.predictions?.base2031 ?? Math.round(carA.askingPrice * 1.20)), b: (carB.predictions?.base2031 ?? Math.round(carB.askingPrice * 1.20)), fmt: fmt, higher: true },
                  { label: "10-Year Value (Base)", a: (carA.predictions?.base2036 ?? Math.round(carA.askingPrice * 1.45)), b: (carB.predictions?.base2036 ?? Math.round(carB.askingPrice * 1.45)), fmt: fmt, higher: true },
                ].map(row => (
