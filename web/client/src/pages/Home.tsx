...
import { CARS_BY_RANK, CARS, WEIGHTS, CARS_MARKET_STATS as STATIC_MARKET_STATS } from "@/data/cars";
import { useLiveListings812 } from "@/hooks/useLiveListings812";
import { WEIGHT_LABELS, WEIGHT_DESCRIPTIONS, WEIGHT_EVIDENCE } from "@/data/cars-weights";
import DealerGate, { isReportUnlocked } from "@/components/DealerGate";
...
const GPF_LABEL: Record<CarSpec["gpfStatus"], { label: string; cls: string }> = {
  none: { label: "PRE-GPF", cls: "check-pass" },
  fitted: { label: "GPF FITTED", cls: "check-fail" },
  borderline: { label: "BORDERLINE", cls: "check-borderline" },
};
...
function IntelBar({ cars }: { cars: CarSpec[] }) {
  const activeCars = cars.filter((c) => !c.soldDate).sort((a, b) => a.rank - b.rank);
      ...
      <div className="container py-2.5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="text-xs font-mono truncate">
            ...
            <span className="hidden sm:inline text-foreground/60">{topCar.gpfStatus === "none" ? "Pre-GPF" : "GPF Fitted"}</span>
          </div>
        </div>
        <Link href={`/car/${topCar.id}`} className="shrink-0 px-3 py-1.5 bg-primary text-primary-foreground text-[10px] font-bold tracking-wide hover:bg-primary/90 transition-colors whitespace-nowrap">
          View Car →
        </Link>
      </div>
    </div>
...
function Hero({ cars, marketStats }: { cars: CarSpec[]; marketStats: { activeListings: number; lastUpdated: string } }) {
  return (
    <section id="overview" className="relative min-h-[85vh] flex items-end overflow-hidden">
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{ backgroundImage: `url(https://d2xsxph8kpxj0f.cloudfront.net/108231505/n92Lo6pqr7S5NaS6XDN7WL/car21_photo1_7848a39a.jpg)` }}
      />
      ...
      <div className="relative container pb-10 pt-28 md:pb-16">
        <div className="max-w-3xl">
          <div className="data-label mb-3 text-primary text-xs">Ferrari Approved UK · {marketStats.activeListings} Active Listings · Refreshed {marketStats.lastUpdated}</div>
          <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl font-black text-foreground leading-tight mb-4 md:mb-6">
            Ferrari 812<br />
            <span className="text-primary italic">Superfast</span>
          </h1>
          <p className="text-base md:text-lg text-foreground/70 max-w-xl mb-6 md:mb-8 font-light">
            A rigorous analysis of every Ferrari 812 Superfast for sale in the UK.
            We build the case from first principles — then reveal the answer.
          </p>
          {/* Key stats — 2-col on mobile, 4-col on md+ — all dynamic from live data */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-border mb-4">
            {((): {label: string; value: string}[] => {
              const activeCars = cars.filter(c => !c.soldDate);
              const prices = activeCars.map(c => c.askingPrice);
              const minP = prices.length ? Math.round(Math.min(...prices) / 1000) : 0;
              const maxP = prices.length ? Math.round(Math.max(...prices) / 1000) : 0;
              const bestGap = activeCars.length ? Math.max(...activeCars.map(c => c.priceVariance)) : 0;
              const bestCar = activeCars.find(c => c.priceVariance === bestGap);
              return [
                { label: "Active Listings", value: String(marketStats.activeListings) },
          ...
          <div className="mb-6 border border-emerald-300 bg-emerald-50 px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div className="text-sm text-emerald-900">
              <strong>Investment verdict: Buy now.</strong> The 812 Superfast is at or near its depreciation floor. Pre-GPF examples (pre-2019) are appreciating. The naturally aspirated V12 is irreplaceable — Ferrari will never build another. Collector demand is rising as the hybrid era begins. Best-spec examples are already trading above list.
            </div>
          </div>
          {/* Mobile CTA row */}
...
function RankingsSection({ cars, marketStats }: { cars: CarSpec[]; marketStats: { activeListings: number; lastUpdated: string; lastUpdatedISO?: string } }) {
  const [showSold, setShowSold] = React.useState(false);
  const [dealerUnlocked, setDealerUnlocked] = useState(() => isReportUnlocked("812-superfast"));
  ...
  const dealerFiltered = dealerFilter === "all" ? cars : cars.filter(c =>
  ...
  const newTodayCount = cars.filter(c => c.firstSeen === todayStr && !c.soldDate).length;
  ...
  const activeCars = [...filteredCars].sort((a, b) => {
    ...
    if (sortKey === "iiv") return ((a.iiv ?? 0) - (b.iiv ?? 0)) * mul;
    if (sortKey === "variance") return (a.priceVariance - b.priceVariance) * mul;
    return 0;
  });
  const soldCount = cars.filter(c => c.soldDate).length;
  const gatedCars = cars.filter(c => !c.soldDate && c.dealerType === "general-dealer");
  const freeCars = cars.filter(c => !c.soldDate && c.dealerType !== "general-dealer");
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-8 md:mb-12">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              All {marketStats.activeListings} cars compared side-by-side. The IIV (Intrinsic Investment Value) shows whether each car is priced above or below fair market value — positive variance means you're buying below IIV.
            </p>
            {/* Social Proof Strip (P10) */}
            <div className="flex flex-wrap items-center gap-3 mt-4">
              {[
                { icon: "◉", label: `${marketStats.activeListings} active listings`, cls: "text-primary" },
                { icon: "◎", label: `Updated ${marketStats.lastUpdated}`, cls: "text-muted-foreground" },
                { icon: "◈", label: "17 scoring criteria", cls: "text-muted-foreground" },
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              All ({cars.filter(c => !c.soldDate).length})
            </button>
            {/* Main Dealer */}
            <button
            ...
            >
              Main Dealer ({cars.filter(c => !c.soldDate && c.dealerType === "ferrari-approved").length})
            </button>
            {/* Independent */}
            <button
            ...
            >
              Independent ({cars.filter(c => !c.soldDate && c.dealerType !== "ferrari-approved").length})
            </button>
            {newTodayCount > 0 && (
              <>
        ...
        <MarketChangeBanner
          cars={cars}
          lastUpdated={marketStats.lastUpdatedISO ?? marketStats.lastUpdated}
          modelName="812 Superfast"
          onCarClick={(id) => { window.location.href = `/car/${id}`; }}
        />
          ...
          <table ref={tableRef} className="w-full text-xs md:text-sm min-w-[640px]">
            ...
            <tbody>
              {activeCars.map((car) => (
                ...
                <tr data-car-id={car.id} onClick={() => { if (!dealerUnlocked && car.dealerType === 'general-dealer') return; window.location.href = `/car/${car.id}`; }} className={`border-b border-border/50 hover:bg-muted/40 transition-all duration-300 ${car.soldDate ? 'opacity-50' : ''} ${!dealerUnlocked && car.dealerType === 'general-dealer' ? 'opacity-40 blur-[1px] select-none pointer-events-none' : 'cursor-pointer'}`}>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {car.motExpired && <span className="px-1.5 py-0.5 text-[9px] font-bold bg-red-50 text-red-700 border border-red-200" title="MOT certificate has expired — buyer must arrange new MOT before road use">⚠ MOT EXPIRED</span>}
                    ...
                    {(car as any).firstSeen && (
                      <div className="text-[9px] text-muted-foreground/60 mt-0.5">First seen: {new Date((car as any).firstSeen + "T12:00:00Z").toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</div>
                    )}
                  </td>
                  ...
                  <td className="p-3 text-center">
                    {car.firstSeen ? (
                      <div className="flex flex-col items-center gap-0.5">
                        ...
                        <span className="text-[10px] text-muted-foreground font-mono">{new Date(car.firstSeen + "T12:00:00Z").toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</span>
                      </div>
                    ) : (
                  ...
                  <td className="p-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      {!dealerUnlocked && car.dealerType === 'general-dealer' ? (
                        <span className="text-muted-foreground/50 text-xs" title="Unlock to view">🔒</span>
                      ) : (
                        <Link href={`/car/${car.id}`} className="text-primary hover:text-primary/80 text-xs font-medium underline underline-offset-2 whitespace-nowrap">
                          View →
                        </Link>
                      )}
        ...
        {/* Dealer gate banner below rankings table */}
        {!dealerUnlocked && gatedCars.length > 0 && (
          <div className="mt-6">
            <DealerGate
              modelKey="812-superfast"
              modelLabel="Ferrari 812 Superfast"
              freeCount={freeCars.length}
              gatedCount={gatedCars.length}
              onUnlock={() => setDealerUnlocked(true)}
            >
              {() => null}
            </DealerGate>
          </div>
...
function CarCard({ car }: { car: CarSpec }) {
  ...
  const gpfInfo = GPF_LABEL[car.gpfStatus];
  return (
    <div className={`car-card bg-card flex flex-col h-full ${car.rank === 1 ? "rank-1" : ""} ${car.soldDate ? 'opacity-60' : ''}`}>
      ...
      <div className="p-4 md:p-5 border-b border-border">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-1.5 mb-1">
              ...
              {car.motExpired && (
                <span className="px-1.5 py-0.5 text-[10px] font-bold bg-red-50 text-red-700 border border-red-200 whitespace-nowrap" title="MOT certificate has expired — buyer must arrange new MOT before road use">
                  ⚠ MOT EXPIRED
                </span>
              )}
              {car.priceDropDate && car.priceDropAmount && (
        ...
        {car.images && car.images.length > 0 && (
          <div className="mt-3 rounded-md overflow-hidden bg-muted" style={{height: '160px'}}>
            <img
              src={car.images[0]}
              alt={`${car.year} Ferrari 812 Superfast ${car.colour}`}
      ...
      <div className="p-4 md:p-5 border-b border-border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5">
          <CheckItem pass={car.checklist.preGPF} label="Pre-GPF" />
          <CheckItem pass={car.checklist.suspensionLift} label="Suspension Lift" />
          <CheckItem pass={car.checklist.carbonSteeringWheel} label="Carbon Steering Wheel" />
          ...
          <CheckItem pass={car.checklist.carbonPack} label="Full Carbon Pack" />
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-1">
              <Link href={`/car/${car.id}`} className="text-primary text-sm font-medium hover:underline flex-1">
...
function CarsSection({ cars }: { cars: CarSpec[] }) {
  ...
  const [dealerUnlocked, setDealerUnlocked] = useState(() => isReportUnlocked("812-superfast"));
  const soldCount = cars.filter((c) => c.soldDate).length;
  // Free tier: Ferrari Approved + independent-specialist (not general-dealer)
  const freeCars = cars.filter(c => !c.soldDate && (c.dealerType === "ferrari-approved" || c.dealerType === "independent-specialist"));
  const gatedCars = cars.filter(c => !c.soldDate && c.dealerType === "general-dealer");
  const allActiveCars = cars.filter(c => !c.soldDate);
  const visibleCars = cars.filter((c) => {
    ...
    if (!dealerUnlocked && c.dealerType === "general-dealer") return false; // hide gated cars
  ...
  const mainDealerCount = cars.filter(c => !c.soldDate && c.dealerType === "ferrari-approved").length;
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-6 md:mb-10">
          ...
          <div className="flex-1 min-w-0">
            <div className="data-label text-primary mb-2">Ferrari Approved UK · {allActiveCars.length} Active Listings</div>
            <h2 className="font-serif text-2xl md:text-4xl font-bold text-foreground mb-3 md:mb-4">
              Browse All Cars
            </h2>
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              Every Ferrari 812 Superfast currently for sale in the UK, ranked by value.
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-border">
          <div className="flex items-center gap-2 flex-wrap">
            {([
              { key: "all", label: "All Sources", count: allActiveCars.length },
              { key: "ferrari-approved", label: "Ferrari Approved", count: cars.filter(c => !c.soldDate && c.dealerType === "ferrari-approved").length },
              { key: "independent-specialist", label: "Independents", count: cars.filter(c => !c.soldDate && (c.dealerType === "independent-specialist" || c.dealerType === "general-dealer")).length },
            ] as const).map(f => (
              <button
                key={f.key}
        ...
        {/* Dealer gate — shown when there are gated cars and user hasn't unlocked yet */}
        {!dealerUnlocked && gatedCars.length > 0 && (
          <DealerGate
            modelKey="812-superfast"
            modelLabel="Ferrari 812 Superfast"
...
const SENTIMENT_BY_YEAR: Record<string, number> = {
  ...
  "2025": 2.00,
  "2026": 1.00,
  // Forward projections — sentiment expected to rise as "last NA V12" narrative matures
  "2028": 1.55,
  "2031": 1.70,
  "2033": 1.80,
  "2036": 1.90,
...
// ─── Predictions ──────────────────────────────────────────────────────────────
function PredictionsSection({ cars }: { cars: CarSpec[] }) {
  const activeCarsForPred = cars.filter(c => !c.soldDate);
        ...
        <div className="flex gap-4 md:gap-8 items-start mb-8 md:mb-12">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              ...
              Base rates derived from Hagerty Market Index, Full Throttle Talk, and comparable Ferrari V12 trajectories.
            </p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 bg-card border border-border p-4 md:p-6">
            ...
            {showSentiment && (
              <p className="text-[10px] text-muted-foreground mt-2 leading-relaxed">
                <span className="font-semibold" style={{ color: "oklch(0.65 0.18 295)" }}>Purple line</span> = influence-weighted sentiment score (right axis, 0–2.0). 2026 data is measured; 2028–2036 projected based on "last NA V12" narrative maturation. Sentiment rising alongside price supports the investment thesis.
              </p>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-card border border-border p-4">
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                <p>Base: <span className="text-foreground font-mono">+4% p.a.</span> (Hagerty V12 index)</p>
...
// ─── Buyer's Guide ────────────────────────────────────────────────────────────
function BuyersGuideSection({ cars }: { cars: CarSpec[] }) {
  const activeCars = cars.filter(c => !c.soldDate);
  ...
  const items = [
    { title: "The GPF Divide", priority: "CRITICAL", priorityCls: "text-red-700 border-red-400/30 bg-red-400/10", content: "Pre-GPF cars (2018–early 2019 builds) produce the pure, unrestricted V12 soundtrack that collectors covet. Always verify the build date, not just the registration year — some 2020-registered cars were built pre-GPF." },
    ...
    { title: "ULEZ & Road Tax", priority: "UK-SPECIFIC", priorityCls: "text-muted-foreground border-border", content: "Despite 366g/km CO2, the 812 Superfast is Euro 6 compliant and fully ULEZ-exempt. Annual VED is £620 (highest band). No daily charge to drive in London or other UK Clean Air Zones." },
    { title: "Known Issues", priority: "INSPECT", priorityCls: "text-amber-700 border-amber-400/30 bg-amber-400/10", content: "Check for: solenoid oil leaks (engine valley), rear window bonding recall (2018–2020), battery drain causing ECU faults, rear-wheel steering warnings, and MagneRide damper wear. A Ferrari dealer PPI is mandatory." },
    { title: "Market Timing", priority: "OPPORTUNITY", priorityCls: "text-green-700 border-green-400/30 bg-green-400/10", content: (() => {
      ...
      return `The 812 Superfast is widely considered at the bottom of its depreciation curve in 2026.With ${activeCars.length} active UK listings ranging from £${Math.round(minP/1000)}k to £${Math.round(maxP/1000)}k, and ${undervaluedCount} currently priced below IIV, the market presents genuine buying opportunities. The 12Cilindri successor costs ~£500k after options. With only ~440 UK examples, the investment window is now.`;
    })() },
  ];
...
// ─── Verdict Section ─────────────────────────────────────────────────────────
function VerdictSection({ cars }: { cars: CarSpec[] }) {
  const [showAll, setShowAll] = useState(false);
  const [listSearch, setListSearch] = useState("");
  const [showSold, setShowSold] = useState(false);
  const soldCount = cars.filter(c => c.soldDate).length;
  const activeCars = showSold ? cars : cars.filter(c => !c.soldDate);
      ...
      <div className="container">
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          <p className="text-base md:text-lg text-muted-foreground max-w-2xl mx-auto">
            ...
            these are the three Ferrari 812 Superfasts worth buying right now.
          </p>
        ...
        <div className="border border-border bg-card">
          ...
          <div className={`divide-y divide-border/50 ${
            ...
            {rest.map((car) => (
              <Link key={car.id} href={`/car/${car.id}`} className={`flex items-center gap-3 px-4 py-3 hover:bg-muted/40 transition-colors group cursor-pointer ${car.soldDate ? 'opacity-50' : ''}`}>
                {/* Rank */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-foreground truncate">
                    ...
                    {car.motExpired && <span className="ml-1.5 text-[9px] font-bold text-red-700">⚠ MOT</span>}
...
const SENTIMENT_DATA = {
  summary: {
    totalSources: 18,
    totalViewsAnalysed: 15_915_374,
    weightedScore: 1.547,  // out of 2.0
    pctPositive: 98.5,
    pctMixed: 1.5,
    pctNegative: 0.0,
  ...
  platformBreakdown: [
    { platform: "YouTube", count: 13, totalViews: 15_494_000, weightedSentiment: 1.63, sentimentPct: 94.2, color: "oklch(0.72 0.12 75)" },
    { platform: "X / Twitter", count: 1, totalViews: 250_000, weightedSentiment: 1.00, sentimentPct: 2.8, color: "oklch(0.55 0.08 75)" },
    { platform: "TikTok", count: 1, totalViews: 107_700, weightedSentiment: 2.00, sentimentPct: 1.4, color: "oklch(0.45 0.06 75)" },
    { platform: "Instagram", count: 1, totalViews: 57_000, weightedSentiment: 1.00, sentimentPct: 0.9, color: "oklch(0.38 0.05 75)" },
    { platform: "Forums", count: 1, totalViews: 6_626, weightedSentiment: 1.00, sentimentPct: 0.5, color: "oklch(0.30 0.04 75)" },
    { platform: "Reddit", count: 1, totalViews: 48, weightedSentiment: 0.00, sentimentPct: 0.2, color: "oklch(0.25 0.03 75)" },
  ],
  yearTrend: [
    { year: 2017, score: 1.46, count: 2 },
    { year: 2018, score: 1.43, count: 2 },
    { year: 2019, score: 2.00, count: 2 },
    { year: 2020, score: 1.69, count: 3 },
    { year: 2021, score: 1.00, count: 1 },
    { year: 2022, score: 1.00, count: 1 },
    { year: 2023, score: 2.00, count: 2 },
    { year: 2024, score: 0.78, count: 2 },
    { year: 2025, score: 2.00, count: 1 },
    { year: 2026, score: 1.00, count: 2 },
  ],
  topThemes: [
    { theme: "V12 Sound & Exhaust Note", count: 8, sentiment: "Strongly Positive", note: "Universally cited as the car's defining characteristic. Pre-GPF models attract premium commentary." },
    { theme: "Driving Feel & Engagement", count: 7, sentiment: "Positive", note: "Praised for balance between GT comfort and supercar intensity. Four-wheel steering divides opinion." },
    { theme: "Future Classic / Investment", count: 5, sentiment: "Positive", note: "Last NA V12 Ferrari GT narrative dominates investment discussion. Competizione creates strong value floor." },
    { theme: "Performance & Acceleration", count: 5, sentiment: "Strongly Positive", note: "0–60 in 2.9s and 211mph top speed consistently impress. Faster than F12 due to shorter gear ratios." },
    { theme: "Design & Styling", count: 4, sentiment: "Positive", note: "Exterior polarising but respected. Interior quality praised; infotainment criticised as dated." },
    { theme: "Electric Power Steering", count: 3, sentiment: "Mixed", note: "Chris Harris and others flag EPS as too quick and lacking connection — the car's most cited weakness." },
    { theme: "Ownership Running Costs", count: 2, sentiment: "Mixed", note: "Real owners note high insurance and maintenance. Recommended only for those who can absorb costs comfortably." },
    { theme: "GPF vs Non-GPF", count: 3, sentiment: "Positive", note: "Pre-2019 non-GPF models preferred for sound drama. Post-GPF still excellent but less theatrical." },
  ],
  topInfluencers: [
    { creator: "Doug DeMuro", platform: "YouTube", subscribers: "5.07M", views: 3_100_000, weight: 48.2, sentiment: "Strongly Positive", quote: "DougScore of 72/100 — tied with the Porsche 918 Spyder. The most impressive V12 two-seater Ferrari in the brand's history.", url: "https://www.youtube.com/watch?v=r5K49HczLvM", year: 2019 },
    { creator: "carwow", platform: "YouTube", subscribers: "10.9M", views: 3_700_000, weight: 47.9, sentiment: "Positive", quote: "The new Ferrari 812 Superfast — here are the top 10 things you need to know about Ferrari's most powerful road car ever.", url: "https://www.youtube.com/watch?v=LQ6n-djlJrc", year: 2017 },
    { creator: "Top Gear / Chris Harris", platform: "YouTube", subscribers: "9.47M", views: 2_700_000, weight: 46.8, sentiment: "Mixed (Positive lean)", quote: "Fantastically fast on track — immense brakes, glorious 800hp V12. But the electric power steering is too quick and lacks connection.", url: "https://www.youtube.com/watch?v=b1YcwlmFEy8", year: 2018 },
    { creator: "Supercar Blondie", platform: "YouTube", subscribers: "22.1M", views: 616_000, weight: 43.2, sentiment: "Strongly Positive", quote: "Organised a surprise for a waiter in Cologne — to drive the Ferrari 812 Superfast before his retirement. The reaction says everything about what this car means to people.", url: "https://www.youtube.com/watch?v=ZwYtnMfMMA0", year: 2020 },
    { creator: "Shmee150", platform: "YouTube", subscribers: "2.85M", views: 1_200_000, weight: 42.5, sentiment: "Strongly Positive", quote: "Utterly blown away. The best super-GT money can buy — a dream and a work of art.", url: "https://www.youtube.com/watch?v=lzyr1-hfuwo", year: 2017 },
    { creator: "Harry's Garage", platform: "YouTube", subscribers: "865K", views: 1_100_000, weight: 40.1, sentiment: "Positive", quote: "A playful, engaging car that keeps urging you on. A true GT that also happens to be wild.", url: "https://www.youtube.com/watch?v=PKK9QvjwmEM", year: 2020 },
    { creator: "Seen Through Glass", platform: "YouTube", subscribers: "1.1M", views: 873_000, weight: 39.8, sentiment: "Strongly Positive", quote: "The Best Sounding Car In The World — the Novitec 812 GTS is an experience unlike any other.", url: "https://www.youtube.com/watch?v=KNqvnCBi07k", year: 2023 },
    { creator: "Rory Reid (AutoTrader)", platform: "YouTube", subscribers: "1.76M", views: 1_000_000, weight: 39.5, sentiment: "Strongly Positive", quote: "Time to savour the speed, noise and beauty. The 812 Superfast is the greatest escape.", url: "https://www.youtube.com/watch?v=7LG8Mc3CY24", year: 2020 },
    { creator: "Evo Magazine", platform: "YouTube", subscribers: "655K", views: 400_000, weight: 37.2, sentiment: "Strongly Positive", quote: "Monumentally fast, sounded absolutely out of this world. A deeply seminal range of abilities on both road and track. An almighty piece of work.", url: "https://www.evo.co.uk/ferrari/812-superfast", year: 2025 },
    { creator: "Mr JWW", platform: "YouTube", subscribers: "1.01M", views: 376_000, weight: 36.9, sentiment: "Strongly Positive", quote: "Sensational. Gear ratios 6% shorter than the F12, with 30% faster upshifts that explode like the LaFerrari.", url: "https://www.youtube.com/watch?v=iFcnKKaA-70", year: 2018 },
  ],
  keyInsights: [
    { label: "Buyer Signal", text: "98.5% of influence-weighted sentiment is positive — the 812 Superfast has no meaningful negative narrative in the influencer space. This supports price resilience.", type: "positive" },
    { label: "Sound Premium", text: "Pre-GPF (pre-2019) models generate disproportionately positive commentary around exhaust note. This translates to a real market premium of ~£5,000–£10,000 vs equivalent post-GPF cars.", type: "positive" },
    { label: "EPS Discount Lever", text: "Electric power steering is the single most cited criticism (Top Gear, Evo, community forums). Use this as a buyer negotiation point — it's a known, acknowledged weakness.", type: "neutral" },
    { label: "Investment Narrative", text: "'Last NA V12 Ferrari GT' is the dominant investment thesis across YouTube and X. The Competizione's £600k+ market price creates a strong value floor for standard 812s.", type: "positive" },
    { label: "2024 Sentiment Dip", text: "Weighted sentiment dipped to 0.78 in 2024 — the lowest in the dataset. This reflects realistic ownership cost discussions and market correction commentary, not fundamental criticism of the car.", type: "neutral" },
  ],
};
      ...
      <div className="container">
        ...
        <div className="mt-6 p-3 border border-border/40 bg-muted/10">
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            <span className="font-semibold text-foreground">Methodology:</span> {summary.methodology} Sources include YouTube reviews, TikTok content, Instagram posts, X/Twitter discussions, Reddit threads, and owner forums. Data collected March 2026.
          </p>
        </div>
      </div>
    </section>
...
// ─── Finance Section ──────────────────────────────────────────────────────────
function FinanceSectionWrapper({ cars }: { cars: CarSpec[] }) {
  const activeCars = cars.filter(c => !c.soldDate);
  const avgPrice = activeCars.length > 0
    ? Math.round(activeCars.reduce((s, c) => s + c.askingPrice, 0) / activeCars.length)
    : 285000;
  const topCar = activeCars[0];
  const iivPrice = topCar && topCar.iiv && topCar.iiv > topCar.askingPrice ? topCar.iiv : undefined;
  return (
      ...
      <div className="container">
        <div className="flex gap-4 md:gap-8 items-start mb-8 md:mb-10">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
              Estimate your monthly PCP or HP payments based on the current average 812 Superfast asking price.
        ...
        <FinanceCalculator
          ...
          iivPrice={iivPrice}
          carTitle="Ferrari 812 Superfast"
        />
      </div>
    </section>
  );
    ...
    <footer className="border-t border-border py-8 md:py-10">
      <div className="container">
        <div className="flex flex-col md:flex-row justify-between gap-6">
          <div>
            <div className="font-serif text-base md:text-lg font-bold text-foreground mb-1">Ferrari 812 Superfast</div>
            <div className="data-label">UK Investment Analysis · March 2026</div>
...
export default function Home() {
  const { listings: liveListings, marketStats: liveStats, isLoading, error } = useLiveListings812();

  // Always use live DB data — no static fallback
  const cars = liveListings;
  const marketStats = liveStats;

  // Show a loading skeleton while the DB query is in flight
  if (isLoading) {
    return (
  ...
  // Show an error state if the DB query failed
  if (error) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center max-w-md">
          <p className="text-sm text-destructive font-medium mb-2">Failed to load market data</p>
  ...
  return (
    <div className="min-h-screen bg-background">
      <IntelBar cars={cars} />
      <GlobalNav reportTitle="812 Superfast" />
      <Hero cars={cars} marketStats={marketStats} />
      <FloatingFAB />
      <SharedFrameworkSection
        ...
        weightEvidence={WEIGHT_EVIDENCE}
        sectionNumber="01"
        modelName="Ferrari 812 Superfast"
      />
      <RankingsSection cars={cars} marketStats={marketStats} />
      {/* <CarsSection /> */}
      <BuyersGuideSection cars={cars} />
      <PredictionsSection cars={cars} />
      <InfluencerSentimentSection />
      <VerdictSection cars={cars} />
      <FinanceSectionWrapper cars={cars} />
      <ResearchHubCTA />
      <Footer />
    </div>
  );
